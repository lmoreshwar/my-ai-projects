...
function buildGeneratePrompt(job, g, snapshot, existing, liveWalk, liveTrace) {
  ...
  return [
    ...
    '- Locators: ONE strategy per element by default — getByTestId is TIER-1 when the app exposes a dedicated test-id attribute (data-test/data-testid; set testIdAttribute in playwright.config), otherwise getByRole/getByLabel/getByPlaceholder. A SmartLocator fallback chain is allowed ONLY for a fragile element and MUST carry a `// reason:` note (max 3 strategies). No stacked speculative locators.',
