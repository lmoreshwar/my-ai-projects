...
function buildPrompt(fw: string, job: CodegenJob, trace: AgentStep[]): string {
  ...
  return [
    ...
    '- LOCATOR STRATEGY (Playwright official priority — pick the HIGHEST-priority strategy the element ACTUALLY supports; fall back only when a higher one does not exist): (1) getByTestId() is the TIER-1 strategy whenever the app tags the element with a dedicated test-id attribute (data-test/data-testid/data-qa) — the most stable choice across builds; set testIdAttribute in playwright.config so getByTestId resolves the app\'s attribute (e.g. \'data-test\'); (2) getByRole(role, { name }) for interactive elements (button/link/textbox/checkbox/radio/menuitem/option) — the primary strategy when the element has NO test-id; (3) getByLabel() for form fields with a real <label>; (4) getByPlaceholder() only when no label/role/testid; (5) getByText({ exact: true }) for static content/links; (6) getByAltText() for images; (7) CSS/XPath LAST RESORT and only SCOPED/chained (e.g. locator(".row", { hasText: "Admin" }).getByRole("textbox")). Default to ONE strategy per element; do NOT stack.',
    '- DYNAMIC / PARAMETERIZED LOCATORS (one getter, not N hardcoded twins): when sibling controls differ ONLY by an id/value embedded in the SAME stable attribute (add-to-cart-<sku>, row-<id>, tab-<name>), emit ONE parameterized getter that takes the varying part — e.g. `addToCartButton = (itemId: string): Locator => this.page.getByTestId(`add-to-cart-${itemId}`)` — instead of a separate hardcoded getter per item. Derive the pattern from the PROVEN live locator (the captured value becomes the argument); never invent ids the evidence did not show. Keep it a SINGLE strategy.',
